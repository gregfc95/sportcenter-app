"""Tests de POST /api/notificaciones/recordatorio-renovacion (disparo manual de demo).

Espejo del test de cupo-disponible: se ejercita vía `test_client` con el header
`X-User-ID` (el stub de auth real) y el mailer monkeypatcheado. Importa el guard
de rol y el armado de los labels (incluida la fecha límite del día 11), no el
transporte.
"""
...
PATH = "/api/notificaciones/recordatorio-renovacion"


def test_client_no_puede_disparar_el_recordatorio(
...
def test_happy_path_envia_el_email_con_los_datos_del_turno(
    ...
    enviados = []
    monkeypatch.setattr(
        "app.services.notificacion_service.send_recordatorio_renovacion_email",
    ...
    # El deadline del abono es el día 11 del mes en curso (dd/mm).
    assert kw["fecha_limite_label"].startswith("11/")
