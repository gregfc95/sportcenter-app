...
export async function listClientes() {
  const res = await fetch(`${USERS_BASE}?role=client`, { headers: authHeaders() });
  const body = await parseJsonSafely(res);
  if (!res.ok) throw toApiError(body, res.status, "No pudimos cargar los clientes.");
  return body;
}
...
export async function createCliente(payload) {
  ...
  const body = await parseJsonSafely(res);
  if (!res.ok) throw toApiError(body, res.status, "No se pudo crear el cliente.");
  return body;
}
